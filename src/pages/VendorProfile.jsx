...
import { vendorProfile, vendorActivityLog } from '../data/mockData';
import {
  User, Mail, Phone, MapPin, Shield, Calendar, Clock,
  Edit2, Lock, Bell, Save, CheckCircle, Activity, Globe, Building
} from 'lucide-react';
...
export default function VendorProfile() {
  ...
  const [notifications, setNotifications] = useState(vendorProfile.notifications);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">
            <span className="text-primary/80">Vendor Identity</span>
            <div className="w-1.5 h-1.5 rounded-full bg-slate-300" />
            <span>Business Management</span>
          </div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Profile & Agency Settings</h2>
        </div>
        {saved && (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden h-fit">
          <div className="p-8 flex flex-col items-center text-center">
            ...
            <div className="w-full mt-8 pt-8 border-t border-slate-50 space-y-4">
              {[
                { icon: Building, label: profile.agencyName },
                { icon: Calendar, label: `Joined ${profile.joinedDate}` },
                { icon: Clock, label: `Last login: ${profile.lastLogin}` },
              ].map(({ icon: Icon, label }) => (
                <div key={label} className="flex items-center gap-3 text-slate-500 text-left">
                  <Icon size={14} className="text-slate-300 flex-shrink-0" />
                  <span className="text-[10px] font-bold uppercase tracking-widest leading-relaxed">{label}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="xl:col-span-3 space-y-6">
          ...
          {activeTab === 'profile' && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-8 py-5 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
                <p className="text-[10px] font-bold text-slate-900 uppercase tracking-[0.3em]">Identity Details</p>
                {!editMode ? (
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setEditMode(false)} className="px-4 py-2 bg-white border border-slate-200 rounded-xl text-[9px] font-bold text-slate-400 uppercase tracking-widest hover:bg-slate-50 transition-all">Cancel</button>
              ...
              <div className="p-8">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  {[
                    ...
                    { label: 'Business Email', key: 'email', icon: Mail, type: 'email' },
                    { label: 'Contact Phone', key: 'phone', icon: Phone, type: 'tel' },
                    { label: 'Agency Name', key: 'agencyName', icon: Building, type: 'text' },
                    { label: 'Office Address', key: 'address', icon: MapPin, type: 'text' },
                  ...
                  <div className="col-span-1 sm:col-span-2">
                    <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">Agency Bio</label>
                    {editMode ? (
                      <textarea
          ...
          {activeTab === 'security' && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-8 py-5 border-b border-slate-50 bg-slate-50/30">
                <p className="text-[10px] font-bold text-slate-900 uppercase tracking-[0.3em]">Account Security</p>
              </div>
              <div className="p-8 space-y-8">
                <div className="bg-amber-50 border border-amber-100/50 rounded-2xl p-5 flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-[11px] font-bold text-amber-900 leading-tight">Password Health</p>
                    <p className="text-[10px] font-bold text-amber-700/60 uppercase tracking-widest mt-1">Your password was last changed 3 months ago</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  {[
                    ...
                    <div key={label}>
                      <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">{label}</label>
                      <input type="password" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary/10 transition-all" placeholder={placeholder} />
                    </div>
                  ))}
          ...
          {activeTab === 'notifications' && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-8 py-5 border-b border-slate-50 bg-slate-50/30">
                <p className="text-[10px] font-bold text-slate-900 uppercase tracking-[0.3em]">Communication Preferences</p>
              </div>
              <div className="divide-y divide-slate-50">
                {[
                  { key: 'emailAlerts', label: 'Email Notifications', desc: 'Direct alerts to your registered business email' },
                  { key: 'smsAlerts', label: 'SMS Alerts', desc: 'Instant text messages for critical lead updates' },
                  { key: 'newLeadInquiry', label: 'New Lead Inquiries', desc: 'Notify when a potential buyer contacts you' },
                  { key: 'propertyViewed', label: 'Property View Milestones', desc: 'Signal when your listings hit view targets' },
                  { key: 'subscriptionRenewal', label: 'Subscription Reminders', desc: 'Alerts for upcoming plan renewals' },
                  { key: 'marketUpdates', label: 'Market Insights', desc: 'Periodic trends and data reports for your area' },
                ].map(({ key, label, desc }) => (
          ...
          {activeTab === 'activity' && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-8 py-5 border-b border-slate-50 bg-slate-50/30 flex items-center justify-between">
                <p className="text-[10px] font-bold text-slate-900 uppercase tracking-[0.3em]">Recent Activity Log</p>
                <div className="text-[9px] font-bold text-primary bg-primary/5 px-2.5 py-1 rounded-md border border-primary/20 shadow-sm uppercase tracking-[0.1em]">{vendorActivityLog.length} Entries</div>
              </div>
              <div className="divide-y divide-slate-50 max-h-[500px] overflow-y-auto scrollbar-hide">
                {vendorActivityLog.map((log, i) => (
                  <div key={log.id} className="flex items-start gap-6 px-8 py-5 hover:bg-slate-50/50 transition-all group">
                    <div className="flex flex-col items-center gap-2 flex-shrink-0">
                      ...
                      {i < vendorActivityLog.length - 1 && <div className="w-px h-10 bg-slate-100" />}
                    </div>
